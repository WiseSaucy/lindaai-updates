...
If a specific pipeline name is provided (e.g., `/linda-pipeline real-estate`), filter leads to that category.
...
**User:** "/linda-pipeline real-estate"

**AI:** Filters deals to only those in the real estate category/pipeline and generates a focused report for that vertical.

## Error Handling

- **If `brain/pipeline/leads/` does not exist or is empty:** Report: "No deal data found. Your pipeline is empty. Use `/linda-leads add [name]` to start tracking deals, or tell me about your current deals and I'll set up the pipeline."
...
- **If `brain/pipeline/kpis.md` does not exist:** Skip KPI cross-referencing. Note: "No KPI file found. Revenue forecasts won't be compared against targets. Run `/linda-kpi setup` to define targets."
