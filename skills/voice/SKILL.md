---
name: voice
description: Toggle LindaAI's voice-speak-every-response hook on or off. Use when Boss says "turn voice on", "turn voice off", "mute LindaAI", "unmute LindaAI", "stop talking", "start talking", "/voice on", "/voice off", or just "/voice" to flip the current state.
---
...
3. Based on the user's request:
   - **"on" / "unmute" / "start talking"** → Add the Stop hook block if missing:
     ```json
     "Stop": [{ "hooks": [{ "type": "command", "command": "/Users/Saucy/.claude/speak-last-response.sh", "async": true }] }]
     ```
...
5. Confirm to Boss in country voice whether voice is now ON or OFF.
...
- Always validate the resulting JSON (`python3 -c "import json; json.load(open('/Users/Saucy/.claude/settings.json'))"`) before finishing.
